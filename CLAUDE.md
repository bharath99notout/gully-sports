...
**Play Store listing art** (feature graphic, phone/tablet screenshots): keep under **`gully-sports/public/playstore/`** (e.g. `feature-graphic-1024x500.png`, `screenshots/`). Regenerate the feature banner with `python3 public/playstore/build_feature_graphic.py` (needs Pillow). **Canonical launcher icons** stay in **`public/icons/`** — copy `icon-512.png` → `gully-sports-twa/store_icon.png` before `bubblewrap build` when the store icon changes. The TWA repo may still hold extra listing copies or experiments; do **not** put GullySports assets only under unrelated Cursor workspaces (e.g. `order-guru` `.cursor/.../assets/`).
...
- **Play Store listing assets:** `gully-sports/public/playstore/` — feature graphic, screenshots, etc.; **canonical raster launcher art** in **`gully-sports/public/icons/`** (`icon-512.png`, maskable variants, `apple-icon.png`). After icon changes, copy `public/icons/icon-512.png` → `gully-sports-twa/store_icon.png` and bump `twa-manifest.json` before `bubblewrap build`.
